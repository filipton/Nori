...
class DeviceSound(context: Context, private val settings: Settings, private val core: () -> Core, private val http: () -> Http) {
    private val store by lazy { context.getSharedPreferences("flint-devices", Context.MODE_PRIVATE) }
    ...
    private suspend fun arrive(output: String) {
        ...
        android.util.Log.i("flint", "device sound: $output -> ${bound?.name ?: "nothing chosen"}")
        if (bound != null) {
        ...
        val created = runCatching { adopt(output, entry) }.getOrElse {
            ...
            android.util.Log.w("flint", "autoeq for $output: ${it.message}")
            post(Offer(output, entry))
            return
        }
        android.util.Log.i("flint", "device sound: applied AutoEQ ${entry.name} to $output")
