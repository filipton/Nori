...
use nori_model::{ListeningStats, Song};
...
pub fn duration(seconds: i64) -> String {
    nori_text::duration(seconds, false)
}

/// The time left in a song, under the seek bar: "-3:07".
#[cfg_attr(feature = "ffi", uniffi::export)]
pub fn duration_left(seconds: i64) -> String {
    nori_text::duration(seconds, true)
}

/// [duration] (or, `left`, [duration_left]) written into a buffer without allocating, for the player's
/// times asked for over JNI once a second. Written by `nori_text`, which the records' own lines use too.
pub use nori_text::write_duration;
...
/// A band's frequency alone, as its label writes it: "63", "1k".
#[cfg_attr(feature = "ffi", uniffi::export)]
...
#[cfg_attr(feature = "ffi", uniffi::export)]
pub fn eq_hz_title(freq: f32) -> String {
    hz_text(freq) + " Hz"
}
...
/// Which service an item comes from, an album card's subtitle and a song row's second line: the words
/// the records carry about themselves, made where the records are.
pub use nori_model::lines::{album_subtitle, provider_of, song_line};
...
mod tests {
    ...
    fn times_decibels_and_captions() {
        ...
        assert_eq!((eq_hz_title(63.0), eq_hz_title(1000.0)), ("63 Hz".into(), "1k Hz".into()));
    ...
    fn records_come_out_of_a_read_with_their_words() {
        ...
        let album: nori_model::Album = serde_json::from_str(r#"{"id":"1","name":"Homogenic","artist":"Björk","year":1997}"#).unwrap();
        assert_eq!(album.subtitle, "Björk · 1997");
        let artist: nori_model::Artist = serde_json::from_str(r#"{"id":"1","name":"Björk","albumCount":1}"#).unwrap();
        assert_eq!(artist.albums_line, "1 album");
        let list: nori_model::Playlist = serde_json::from_str(r#"{"id":"1","name":"Mix","songCount":12,"duration":2890}"#).unwrap();
