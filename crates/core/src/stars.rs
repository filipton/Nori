...
use crate::client::Starrable;
use crate::{Album, Artist, Song, Starred};
...
pub(crate) fn with_marks<R>(f: impl FnOnce(&HashMap<String, bool>) -> R) -> R {
...
impl<'a> Marks<'a> {
    pub(crate) fn new(marks: &'a HashMap<String, bool>) -> Self {
    ...
    pub(crate) fn kept(&mut self, param: &str, id: &str) -> bool {
        if !self.any_removed {
            return true;
        }
        self.key.clear();
...
fn overlay(starred: Starred, marks: &HashMap<String, bool>) -> Starred {
    ...
    Starred { artists: artists(starred.artists, &mut m), albums: albums(starred.albums, &mut m), songs: songs(starred.songs, &mut m) }
}

/// The favourites screen: artists, albums and songs this session unstarred are left out.
#[uniffi::export]
...
mod tests {
    ...
    fn only_an_unstarred_mark_removes() {
        ...
        let starred = Starred { artists: vec![r("1"), r("2")], albums: vec![a("1"), a("2")], songs: vec![s("1"), s("2"), s("3")] };
