...
impl From<crate::CoreError> for NetError {
    fn from(e: crate::CoreError) -> Self {
        match e {
            crate::CoreError::Api { code, reason } => NetError::Api { code, reason },
            crate::CoreError::Parse { reason } => NetError::Parse { reason },
            crate::CoreError::Db { reason } => NetError::Db { reason },
        }
    }
}
...
pub(crate) async fn get(transport: &dyn Transport, url: String, timeout_ms: u32) -> Result<Vec<u8>, NetError> {
