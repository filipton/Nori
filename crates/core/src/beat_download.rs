...
use nori_automix::beat_model::{self, State, BYTES, SHA256, URL};
use sha2::{Digest, Sha256};

/// How long the whole file may take.
const TIMEOUT_MS: u32 = 120_000;

/// The model's file, fetched first when it is not on the device yet and the network allows. None when it is not
...
pub fn ensure() -> Option<PathBuf> {
    ...
    let got = block_on(nori_net::transport::get(&*client.transport, URL.to_string(), TIMEOUT_MS)).map_err(|e| e.to_string()).and_then(|body| {
        if body.len() as u64 != BYTES || hex(&Sha256::digest(&body)) != SHA256 {
            return Err("the file that arrived was not the right one".to_string());
        }
        ...
        std::fs::write(&part, &body).and_then(|_| std::fs::rename(&part, &file)).map_err(|e| e.to_string())
    });
    match got {
        ...
        Err(reason) => {
            crate::alog::info(&format!("beat model download failed: {reason}"));
            beat_model::set_state(State::Failed(reason));
            None
        }
    }
}
