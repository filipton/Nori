...
/// The app's own tables: carried over from the file that was the default server's.
const APP_TABLES: [&str; 2] = ["autoeq", "profiles"];
/// Indexes of the old layout, named like the new ones.
const OLD_INDEXES: [&str; 6] = ["plays_started", "items_genre", "items_artist", "items_year", "items_starred", "items_rated"];

/// The app's database, opened for `server`'s rows. The first time, the old layout - one file per server,
/// no `server` column - is carried over into it: the file itself (the default server's) and every
/// `nori-<id>.db` beside it, which then go.
pub fn open(path: &str, server: &str) -> rusqlite::Result<Connection> {
    ...
    if !path.is_empty() {
        merge_files(&c, std::path::Path::new(path));
    }
    Ok(c)
}

fn columns(c: &Connection, schema: &str, table: &str) -> rusqlite::Result<Vec<String>> {
    c.prepare(&format!("PRAGMA {schema}.table_info({table})"))?.query_map([], |r| r.get(1))?.collect()
}

fn has_table(c: &Connection, schema: &str, table: &str) -> rusqlite::Result<bool> {
    Ok(!columns(c, schema, table)?.is_empty())
}

/// Copies `schema.from`'s rows into `into` as `server`'s: the columns both have, the new ones defaulted,
/// rowids new unless `keep_rowid`. A row already there stays.
fn copy(c: &Connection, schema: &str, from: &str, into: &str, server: Option<&str>, keep_rowid: bool) -> rusqlite::Result<()> {
    let old = columns(c, schema, from)?;
    let new = columns(c, "main", into)?;
    let list = new.iter().filter(|n| *n != "server" && (keep_rowid || *n != "rowid") && old.contains(n)).cloned().collect::<Vec<_>>().join(", ");
    let (head, value) = match server {
        Some(s) => ("server, ".to_string(), format!("'{}', ", s.replace('\'', "''"))),
        None => (String::new(), String::new()),
    };
    c.execute_batch(&format!("INSERT OR IGNORE INTO main.{into}({head}{list}) SELECT {value}{list} FROM {schema}.{from}"))
}

/// The old layout in this file (the default server's) moved into the new one.
fn upgrade(c: &Connection) -> rusqlite::Result<()> {
    c.execute_batch("BEGIN IMMEDIATE")?;
    let done = (|| {
        let mut moved = Vec::new();
        for t in SERVER_TABLES {
            let cols = columns(c, "main", t)?;
            if !cols.is_empty() && !cols.iter().any(|n| n == "server") {
                moved.push(t);
            }
        }
        if !moved.is_empty() {
            for i in OLD_INDEXES {
                c.execute_batch(&format!("DROP INDEX IF EXISTS {i}"))?;
            }
            for t in &moved {
                c.execute_batch(&format!("ALTER TABLE {t} RENAME TO old_{t}"))?;
            }
        }
        c.execute_batch(SCHEMA)?;
        for t in moved {
            // Items keep their rowids, so the search index (keyed by them) stays as it is.
            copy(c, "main", &format!("old_{t}"), t, Some("default"), t == "items")?;
            c.execute_batch(&format!("DROP TABLE old_{t}"))?;
        }
        Ok(())
    })();
    c.execute_batch(if done.is_ok() { "COMMIT" } else { "ROLLBACK" })?;
    done
}

/// Every other server's old file beside `path` carried in and removed. One that cannot be read is left
/// where it is and tried again next time.
fn merge_files(c: &Connection, path: &std::path::Path) {
    let Some(dir) = path.parent() else { return };
    let Ok(entries) = std::fs::read_dir(dir) else { return };
    for e in entries.flatten() {
        let name = e.file_name().to_string_lossy().to_string();
        let Some(id) = name.strip_prefix("nori-").and_then(|n| n.strip_suffix(".db")) else { continue };
        if id.is_empty() {
            continue;
        }
        let file = e.path();
        match merge(c, &file, id) {
            Ok(()) => {
                for suffix in ["", "-wal", "-shm"] {
                    let _ = std::fs::remove_file(format!("{}{suffix}", file.display()));
                }
                crate::alog::info(&format!("db: {name} carried into the app's database"));
            }
            Err(e) => crate::alog::info(&format!("db: could not carry {name} over: {e}")),
        }
    }
}

fn merge(c: &Connection, file: &std::path::Path, server: &str) -> rusqlite::Result<()> {
    c.execute("ATTACH DATABASE ?1 AS old", [file.display().to_string()])?;
    c.execute_batch("BEGIN IMMEDIATE")?;
    let done = (|| {
        for t in SERVER_TABLES {
            if !has_table(c, "old", t)? {
                continue;
            }
            if t == "items" {
                copy(c, "old", t, t, Some(server), false)?;
                // The search index follows the items to their new rowids.
                if has_table(c, "old", "fts")? {
                    c.execute(
                        "INSERT INTO fts(rowid, text) SELECT n.rowid, f.text FROM old.fts f JOIN old.items o ON o.rowid=f.rowid
                         JOIN items n ON n.server=?1 AND n.kind=o.kind AND n.id=o.id",
                        [server],
                    )?;
                }
            } else {
        ...
        for t in APP_TABLES {
            if has_table(c, "old", t)? {
                copy(c, "old", t, t, None, false)?;
            }
        }
        Ok(())
    })();
    c.execute_batch(if done.is_ok() { "COMMIT" } else { "ROLLBACK" })?;
    c.execute_batch("DETACH DATABASE old")?;
    done
}

/// The app's database for what is the app's alone (the settings): nothing carried over, no server.
pub fn open_app(path: &str) -> rusqlite::Result<Connection> {
    let c = if path.is_empty() { Connection::open_in_memory()? } else { Connection::open(path)? };
    c.execute_batch(PRAGMAS)?;
    c.execute_batch("CREATE TABLE IF NOT EXISTS settings(key TEXT PRIMARY KEY, value TEXT NOT NULL) WITHOUT ROWID;")?;
    Ok(c)
}
...
mod tests {
    ...
    /// The layout before one database: one file per server, no `server` column.
    fn old_file(path: &std::path::Path, song: &str, title: &str) {
        let c = Connection::open(path).unwrap();
        c.execute_batch(
            "CREATE TABLE items(rowid INTEGER PRIMARY KEY, kind INTEGER NOT NULL, id TEXT NOT NULL, json TEXT NOT NULL, UNIQUE(kind, id));
             CREATE VIRTUAL TABLE fts USING fts5(text, tokenize='unicode61 remove_diacritics 2', prefix='2 3');
             CREATE TABLE downloads(id TEXT PRIMARY KEY, json TEXT NOT NULL, ts INTEGER NOT NULL, done INTEGER NOT NULL DEFAULT 0) WITHOUT ROWID;
             CREATE TABLE profiles(name TEXT PRIMARY KEY, json TEXT NOT NULL, outputs TEXT NOT NULL DEFAULT '') WITHOUT ROWID;
             CREATE INDEX items_year ON items(json_extract(json,'$.year')) WHERE kind=2;",
        )
        .unwrap();
        // A gap in the rowids, so a carried-over row would land on the wrong search entry if they were not remapped.
        c.execute("INSERT INTO items(rowid, kind, id, json) VALUES(7, 2, ?1, ?2)", params![song, format!(r#"{{"id":"{song}","title":"{title}"}}"#)]).unwrap();
        c.execute("INSERT INTO fts(rowid, text) VALUES(7, ?1)", [title]).unwrap();
        c.execute("INSERT INTO downloads(id, json, ts, done) VALUES(?1, '{}', 1, 1)", [song]).unwrap();
        c.execute("INSERT INTO profiles(name, json) VALUES(?1, '{}')", [title]).unwrap();
    }

    #[test]
    fn every_servers_old_file_is_carried_into_one_database() {
        let dir = std::env::temp_dir().join(format!("nori-one-db-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        let main = dir.join("nori.db");
        old_file(&main, "a1", "Airbag");
        old_file(&dir.join("nori-x1.db"), "b1", "Bones");
        let path = main.display().to_string();

        let def = open(&path, "default").unwrap();
        assert!(!dir.join("nori-x1.db").exists(), "the other server's file goes once carried over");
        let songs = |c: &Connection| search(c, "o", 10).unwrap().songs.into_iter().map(|s| s.id).collect::<Vec<_>>();
        ...
        let x1 = open(&path, "x1").unwrap();
        assert_eq!(search(&x1, "bones", 10).unwrap().songs[0].id, "b1");
        assert!(songs(&x1).iter().all(|id| id == "b1"));
        let done = |c: &Connection| c.query_row("SELECT count(*) FROM downloads WHERE server=sid() AND done=1", [], |r| r.get::<_, i64>(0)).unwrap();
        assert_eq!((done(&def), done(&x1)), (1, 1));
        let profiles: i64 = def.query_row("SELECT count(*) FROM profiles", [], |r| r.get(0)).unwrap();
        assert_eq!(profiles, 2, "sound profiles are the app's");

        forget_server(&def, "x1").unwrap();
        assert!(search(&x1, "bones", 10).unwrap().songs.is_empty());
        assert_eq!(search(&def, "airbag", 10).unwrap().songs.len(), 1);
        drop((def, x1));
        // Opened again: nothing to carry over, nothing lost.
        assert_eq!(search(&open(&path, "default").unwrap(), "airbag", 10).unwrap().songs.len(), 1);
        let _ = std::fs::remove_dir_all(&dir);
    }
}
