...
//! every route here reads the library, so this never makes octo-fiesta download a provider track. The
//! platform decides when (the last song, or one song left) and appends what comes back.
...
impl Client {
    ...
    async fn first(&self, read: Read) -> Got<Page> {
