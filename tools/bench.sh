...
# utime+stime ticks and voluntary context switches, summed over all threads of all processes
sample() {
  adb shell "t=0; w=0; for p in $(pids); do for d in /proc/\$p/task/*; do
      set -- \$(cat \$d/stat 2>/dev/null | sed 's/.*) //'); t=\$((t + \${12:-0} + \${13:-0}));
      v=\$(grep -s voluntary_ctxt \$d/status | head -1 | tr -dc 0-9); w=\$((w + \${v:-0})); done; done; echo \$t \$w" | tr -d '\r'
}
threads() {
